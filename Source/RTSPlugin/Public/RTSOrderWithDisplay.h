...
class RTSPLUGIN_API URTSOrderWithDisplay : public URTSOrder
{
    GENERATED_BODY()
...
private:
    ...
    TObjectPtr<UTexture2D> NormalIcon;

    /** Hovered icon of the order. Can be shown in the UI. */
    UPROPERTY(Category = "RTS", EditDefaultsOnly, BlueprintReadOnly, meta = (AllowPrivateAccess = true))
    TObjectPtr<UTexture2D> HoveredIcon;

    /** Pressed icon of the order. Can be shown in the UI. */
    UPROPERTY(Category = "RTS", EditDefaultsOnly, BlueprintReadOnly, meta = (AllowPrivateAccess = true))
    TObjectPtr<UTexture2D> PressedIcon;
