...
void URTSOrderHelper::CreateIndividualTargetLocations(TSoftClassPtr<URTSOrder> OrderType,
    const TArray<AActor*>& OrderedActors, const FRTSOrderTargetData& TargetData, TArray<FVector2D>& OutTargetLocations)
{
    if (OrderType == nullptr)
    {
        return;
...
void URTSOrderHelper::ClearOrderQueue(AActor* OrderedActor)
{
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("URTSOrderHelper::EnqueueOrder: The specified actor is invalid."));
        return;
    }
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("URTSOrderHelper::EnqueueOrder: The specified actor has no order component."));
        return;
    }
...
void URTSOrderHelper::InsertOrderAfterCurrentOrder(AActor* OrderedActor, const FRTSOrderData& Order)
{
        if (!IsValid(OrderedActor))
        {
            UE_LOG(LogTemp, Error, TEXT("URTSOrderHelper::InsertOrderAfterCurrentOrder: The specified actor is invalid."));
            return;
        }
...
UTexture2D* URTSOrderHelper::GetNormalIcon(TSoftClassPtr<URTSOrder> OrderType, const AActor* OrderedActor, int32 Index)
{
    if (OrderType == nullptr)
    {
        return nullptr;
...
UTexture2D* URTSOrderHelper::GetHoveredIcon(TSoftClassPtr<URTSOrder> OrderType, const AActor* OrderedActor, int32 Index)
{
    if (OrderType == nullptr)
    {
        return nullptr;
...
UTexture2D* URTSOrderHelper::GetPressedIcon(TSoftClassPtr<URTSOrder> OrderType, const AActor* OrderedActor, int32 Index)
{
    if (OrderType == nullptr)
    {
        return nullptr;
...
UTexture2D* URTSOrderHelper::GetDisabledIcon(TSoftClassPtr<URTSOrder> OrderType, const AActor* OrderedActor,
    int32 Index)
{
    if (OrderType == nullptr)
    {
...
FText URTSOrderHelper::GetName(TSoftClassPtr<URTSOrder> OrderType, const AActor* OrderedActor, int32 Index)
{
    if (OrderType == nullptr)
    {
...
FText URTSOrderHelper::GetDescription(TSoftClassPtr<URTSOrder> OrderType, const AActor* OrderedActor, int32 Index)
{
    if (OrderType == nullptr)
    {
    ...
    const FRTSOrderTargetData& TargetData, int32 Index)
{
    if (!IsValid(OrderedActor))
    {
        UE_LOG(LogTemp, Error, TEXT("URTSOrderHelper::GetOrderTargetScore: The specified actor is invalid."));
        return false;
    }
    ...
    int32 Index)
{
    if (!IsValid(OrderedActor))
    {
        UE_LOG(LogTemp, Error, TEXT("URTSOrderHelper::IsHumanPlayerAutoOrder: The specified actor is invalid."));
        return false;
    }
...
bool URTSOrderHelper::IsAIPlayerAutoOrder(TSoftClassPtr<URTSOrder> OrderType, const AActor* OrderedActor, int32 Index)
{
    if (!IsValid(OrderedActor))
    {
        UE_LOG(LogTemp, Error, TEXT("URTSOrderHelper::IsAIPlayerAutoOrder: The specified actor is invalid."));
        return false;
    }
    ...
    bool bEnable)
{
    if (!IsValid(OrderedActor))
    {
        UE_LOG(LogTemp, Error, TEXT("URTSOrderHelper::SetHumanPlayerAutoOrderState: The specified actor is invalid."));
        return;
    }
...
AActor* URTSOrderHelper::FindTargetForOrderInChaseDistance(TSoftClassPtr<URTSOrder> OrderType,
    const AActor* OrderedActor, int32 Index, float AcquisitionRadius, float ChaseDistance,
    const FVector& OrderedActorHomeLocation, float& OutScore)
{
    ...
    ERTSOrderTargetType TargetType = Order->GetTargetType(OrderedActor, Index);
    if (TargetType == ERTSOrderTargetType::NONE || TargetType == ERTSOrderTargetType::PASSIVE)
    {
        return nullptr;
    }
...
AActor* URTSOrderHelper::FindMostSuitableActorToObeyTheOrder(TSoftClassPtr<URTSOrder> OrderType,
    const TArray<AActor*> OrderedActors, const FRTSOrderTargetData TargetData, int32 Index)
{
    TArray<TTuple<AActor*, float>> ActorsWithScore;
    for (AActor* Actor : OrderedActors)
    {
        if (URTSOrderHelper::CanObeyOrder(OrderType, Actor))
        {

            // This actor is valid. Store it and its score in the array.
            ActorsWithScore.Emplace(Actor, GetOrderTargetScore(OrderType, Actor, TargetData));
        }
    }
...
void URTSOrderHelper::FindActorsInChaseDistance(UObject* WorldContextObject, float AcquisitionRadius,
    float ChaseDistance, const FVector& OrderedActorLocation, const FVector& OrderedActorHomeLocation,
    ...
    const TArray<AActor*> Targets, int32 Index, float& OutScore)
{
    ...
    for (AActor* Actor : Targets)
    {
        ...
        FRTSOrderTargetData OrderTargetData(Actor, FVector(Actor->GetActorLocation()), TargetTags);
